...
vertCuts = Workplane( 'XY' ).box(
    cell_size - 2 * strut_diameter , cell_size - 2 * strut_diameter, plate_height )

additional_box = Workplane( 'XY' ).box( cube_length, cube_length, plate_height )

additional_box = additional_box.cut(
    Workplane( 'XY' ).pushPoints( pos ).eachpoint( lambda u: vertCuts.val().located( u ) ) )

qos = [ ( ( cell_size - cube_length ) / 2 + x * cell_size, 0. , - plate_height / 2 )
        for x in range( int( cube_length / cell_size ) ) ]

horzCuts = Workplane( 'XZ' ).cylinder( cube_length, ( cell_size - 2 * strut_diameter ) / 2. )
horzCuts = Workplane( 'XZ' ).pushPoints( qos ).eachpoint( lambda u: horzCuts.val().located( u ) )
additional_box = additional_box.cut( horzCuts )
...
exporters.export( lattice, f"compressiontest_specimen_sla_{cell_type}.STL" )
